    ...
    <template>
        ...
        <table class="fixed" border="1" width="100%">
          ...
          <th><div class="price" style="background-color: #ccc; ">Total Price</div></th>
            ...
            <tr v-show="showcart" v-for="(landscapepainting, index) in get_all_items_in_cart()" :key="index" >
            <td width="20%"><img class="items" :src = "landscapepainting.imgfirst" style="background-color: #aaa;"></td>
            <td width="20%"> <div class="name" style="background-color: #bbb;">{{landscapepainting.title}}</div></td>
            <td width="20%"><div class="size" style="background-color: #ccc;">{{landscapepainting.size}}</div></td>
            <td width="20%"><input class="number" type="number"  v-model="number" :id="'landscapepaintingcart_qty_'+landscapepainting.id" @input="update_cart(landscapepainting)" style="background-color: #ccc;" title="Number of pieces"></td>
            <td width="20%"><div class="pricepi" style="background-color: #ccc;">{{landscapepainting.price}}</div></td>
            <td width="20%"><div class="price"  style="background-color: #ccc; font-size:x-large; font-weight:500;">{{landscapepainting.total_amount}}</div></td>
            <!-- <td width="20%"><button class="removeitem" :id= "`cart_`+landscapepainting.id " @click="removeitems(landscapepainting)" style="background-color: #ccc;">Remove Item</button></td> -->
            </tr>
            </table>
...
filtered_get_all_items_in_cart(){return this.cartpostprofile.filter((cartpostprofile)=>cartpostprofile.addtocart)},
 ...
 methods:{
          ...
          //   data.forEach( (landscapepainting, index ) => {
          //     content+=
          //         `
          //             <img class="items" src="${landscapepainting.imgfirst}" style="background-color: #aaa;">
          //                 <div class="name" style="background-color: #bbb;">${landscapepainting.title}</div>
          //                 <div class="size" style="background-color: #ccc;">${landscapepainting.size}</div>
          //                   <input class="number" type="number"  v-model="number" id="landscapepaintingcart_qty_${landscapepainting.id}" @input="update_cart(${landscapepainting})" style="background-color: #ccc;" title="Number of pieces">
          //                   <div class="pricepi" style="background-color: #ccc;">${landscapepainting.price}</div>
          //                   <div class="price"  style="background-color: #ccc; ">${landscapepainting.total_amount}</div>
          //           `;
          //   });
         ...
         get_all_items_in_cart(){
           var landscapepainting = [];
           for( var key in localStorage ){
            if( key.split('_')[0] == 'cart'){
              landscapepainting.push( JSON.parse(localStorage.getItem(key)));
               
            }
           }
           return landscapepainting; 
         },
       ...
       removeitems(landscapepainting){
        
        localStorage.removeItem(`cart_${landscapepainting.id}` );
        this.showcart = false},
       ...
       // var cartstorage=JSON.stringify(cartpost);
        // localStorage.setItem(`cartremoval`,cartstorage);
         ...
         update_cart(landscapepainting=''){
          
          var new_amount = '',
              updated_qty = document.querySelector(`#landscapepaintingcart_qty_${landscapepainting.id}`).value,
                //get the existing data from localStorage and update it with the new data;
              existing_data = JSON.parse( localStorage.getItem(`cart_`+landscapepainting.id) );
              // updated_amount=JSON.parse(localStorage.getItem(`landscapepaintingcart_`+landscapepainting.total_amount));
              //return console.log( updated_qty );
                if( existing_data ){
                   new_amount = updated_qty * existing_data.price;                  
                   existing_data['qty'] = updated_qty;
                   existing_data['total_amount']= new_amount; 
                   localStorage.setItem(`cart_`+landscapepainting.id, JSON.stringify( existing_data )); 
                };
                console.log()
